...
class Database():
    def __init__(self):
        self.db= pymysql.connect(host='localhost', port= 3307, user='root', password='123456', db='space', charset='utf8')
...
# CUSTOMER INFORMATION
    ...
    def showItemOrder(self, index, order):
        sql = "SELECT Brand, ProductName, Price FROM PRODUCT ORDER BY " + index + " " + order
    ...
    def showStyleOrder(self, index, order):
        sql = "SELECT StyleID ,ADMIN_AdminID, Description FROM STYLE ORDER BY " + index + " " + order
        num = self.cursor.execute(sql)
        data = self.cursor.fetchall()
        return num, data
    ...
    def getInventoryItem(self, pivot, productName):
        sql = "SELECT distinct(" + pivot + ") FROM inventory where product_productName = %s;"
    ...
    def getCategory(self, category):
        sql = "SELECT Brand, ProductName, Price FROM PRODUCT WHERE Category = \"" + category +"\""
        num = self.cursor.execute(sql)
        data = self.cursor.fetchall()
        return num, data
    ...
    def getReview(self, productName):
        sql = "SELECT CUSTOMER_Customer_ID, Review FROM TRANS JOIN TRANS_has_INVENTORY ON TRANS_idTRANS = idTRANS WHERE REVIEW IS NOT NULL AND INVENTORY_idINVENTORY IN(SELECT idINVENTORY FROM INVENTORY WHERE PRODUCT_ProductName= \"" + productName + "\")"
        num = self.cursor.execute(sql)
        data = self.cursor.fetchall()
        return num, data
    ...
    def getinfluencer_Style(self, id):
        sql = "SELECT StyleID ,ADMIN_AdminID, Description FROM STYLE WHERE ADMIN_AdminID = \"" + id + "\""
        num = self.cursor.execute(sql)
        data = self.cursor.fetchall()
        return num, data
    ...
    def PRODUCT_StyleID(self, productName):
        sql = "SELECT StyleID ,ADMIN_AdminID, Description FROM STYLE WHERE StyleID IN (SELECT STYLE_StyleID FROM STYLE_has_PRODUCT WHERE PRODUCT_ProductName = \"" + productName + "\")"
        num = self.cursor.execute(sql)
        data = self.cursor.fetchall()
        return num, data
    ...
    def Style_PRODUCT(self, styleid):
        sql = "SELECT ProductName, Brand, Price FROM PRODUCT WHERE productName in (SELECT PRODUCT_productName FROM space.style_has_product WHERE STYLE_styleID = " + str(styleid) +" )"
        num = self.cursor.execute(sql)
        data = self.cursor.fetchall()
        return num, data
    ...
    def countCumSold(self, id):
        sql = "SELECT SUM(Quantity) AS sum FROM TRANS_has_INVENTORY WHERE TRANS_idTRANS IN (SELECT idTRANS FROM TRANS WHERE Cancellation = 'No') AND INVENTORY_idINVENTORY IN (SELECT idINVENTORY FROM INVENTORY WHERE PRODUCT_ProductName = \"" + id + "\")"
        num = self.cursor.execute(sql)
        data = self.cursor.fetchall()
        return num, data
    ...
    def insertTrans(self, cusid, payment, totalPrice, dateSold, deliveryAddress):
        ...
        result = self.cursor.execute(sql,(cusid, payment, totalPrice, dateSold, deliveryAddress))
        self.db.commit()
        print(cusid + " : " + payment + "," + str(totalPrice) + "," + dateSold + "," + deliveryAddress + " 구매완료")
    ...
    def getTransID(self, cusid, payment, totalPrice, dateSold, deliveryAddress):
        ...
        num = self.cursor.execute(sql,(cusid, payment, totalPrice, dateSold, deliveryAddress))
        data = self.cursor.fetchone()
        return num, data
